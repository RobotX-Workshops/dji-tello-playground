...
- `base_gamepad_adapter.py` — `BaseGamepadTelloAdapter`, the shared button/axis-mapping logic (Y/A/B, D-pad flips, bumpers, sticks) that every gamepad adapter above except `keyboard_controller.py` subclasses, since they all expose the same `GameControllerState` shape.
