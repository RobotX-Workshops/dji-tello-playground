...
`16_color_contour_detector.py` is the color-tracking analogue of the circle detector — see [`object_detection/README.md`](../object_detection/README.md) for the detection logic it wires up. Unlike `15_circle_dectector.py`, its `DEBUG_MODE` and `NO_TAKEOFF` are plain constants you edit in the script rather than an environment variable, and its defaults are the other way around: `DEBUG_MODE = False` (drone camera by default) and `NO_TAKEOFF = True` (stays grounded by default).
